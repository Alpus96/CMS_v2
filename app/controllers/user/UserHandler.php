<?php
    ...
    * */
    ...
    require_once ROOT_PATH.'app/models/user/UserModel.php';
    ...
    class UserHandler extends UserModel {
        ...
        * */
        function login ($username, $password) {
            ...
            return self::$token_store->create($user->username, $user);
        }
        ...
        * */
        function verifyToken ($token) {
            //  NOTE: return a new token?
            $user = self::$token_store->verify($token);
            if ($user) { return self::$token_store->update($token, $user); } 
            return false;
        }
        ...
        * */
        function logout ($token) {
            //  Destroy the registered token if it exists.
            if ($user = self::$token_store->verify($token))
            { return self::$token_store->destroy($token, $user->username); }
            else { return true; }
        }
